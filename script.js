const app = {
    ...
    // --- NAVEGACIÓN (FIX: Función simplificada) ---
    toggleView: function(viewName) {
        ...
        if (viewName === 'gestion') {
            ...
            this.renderGestion(); // Renderizar editor al entrar
        } else {
            // Ir a Dashboard
            gestion.classList.add('hidden');
            dashboard.classList.remove('hidden');
            this.renderDashboard(); // Refrescar gráficos al salir
        }
    },
    ...
    renderDashboard: function() {
        ...
        this.data.forEach((p, idx) => {
            ...
            // HTML Tareas (Solo preview 3 items)
    ...
    // NOTA: Usamos "Harvesting" (Cosecha) para guardar lo que hay en pantalla antes de añadir/borrar
    // Esto evita que pierdas lo que estabas escribiendo al hacer clic en añadir tarea.
    
    harvestData: function() {
        const adminContainer = document.getElementById('admin-container');
        if(!adminContainer || adminContainer.innerHTML === "") return; // No hacer nada si no estamos en vista gestión
    ...
    addTask: function(pIdx) {
        this.harvestData(); // Guardar estado actual
    ...
    saveData: async function() {
        this.harvestData(); // 1. Cosechar últimos datos de pantalla
        
        // 2. Guardar Local
        localStorage.setItem('cengobData', JSON.stringify(this.data));

        // 3. Guardar Nube (Si existe config)
        ...
        this.toggleView('dashboard'); // Volver al inicio
    },
